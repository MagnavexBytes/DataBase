import pprint
import sqlite3
conn = sqlite3.connect('Lab28/BD_Books.sqlite')
c = conn.cursor()
...
c.execute('''drop trigger if exists after_insert_items''')
c.execute('''drop trigger if exists after_update_items''')
c.execute('''drop trigger if exists after_delete_items''')
...
c.execute('''
          ...
          e_addr varchar(50),
              ...
              constraint FK_employees_e_fname foreign key(e_post) references posts (p_id) on delete set default on update set default,
...
c.execute('''create table if not exists books (
          ...
          b_rest int(5), 
              ...
              constraint FK_books_man foreign key(b_man) references employees(e_tab) on delete set null,
...
c.execute('''create table if not exists orders (
          ...
          o_ready date,
              ...
              constraint FK_orders_company foreign key(o_company) references customers(c_id) on delete cascade on update cascade)
''')
...
c.execute('''create table if not exists items (
          ...
          i_count int(4) not null,
              ...
              constraint FK_items_id foreign key(i_id) references orders(o_id) on delete set null,
...
c.execute('''create table if not exists editors (
          ...
          e_id int(4),
              ...
              constraint FK_editors_id foreign key(e_id) references employees(e_tab) on delete set null)
''')
...
# Триггеры
c.execute('''CREATE trigger after_insert_items
          after insert on items for each row 
          /* when (Select b_rest from books where b_contract = NEW.i_contract) >= NEW.i_count */
          begin
              Select raise (abort, "Слишком много книг") from books where b_contract = NEW.i_contract and b_rest < NEW.i_count;
              update books set b_rest = b_rest - NEW.i_count
              where books.b_contract = NEW.i_contract /* and b_rest >= NEW.i_count */;
              insert into A values(null, NEW.i_id, null, NEW.i_contract, null, NEW.i_count, datetime('now'), 'insert');
          end
''')

c.execute('''CREATE trigger after_update_items
          after update on items for each row
          when (Select b_rest from books where b_contract = NEW.i_contract and NEW.i_contract = OLD.i_contract) >= NEW.i_contract - OLD.i_contract or
              (Select b_rest from books where b_contract = NEW.i_contract and NEW.i_contract != OLD.i_contract) >= NEW.i_contract
            
          begin
              update books set b_rest = b_rest + OLD.i_count where books.b_contract = OLD.i_contract;
              update books set b_rest = b_rest - NEW.i_count where books.b_contract = NEW.i_contract;
              insert into A values(OLD.i_id, NEW.i_id, OLD.i_contract, NEW.i_contract, OLD.i_count, NEW.i_count, datetime('now'), 'update');
          end
''')

c.execute('''CREATE trigger after_delete_items
...
#c.execute('''Update customers set c_id=c_id*10 where c_id<>5''')
...
# После вставки в items
pp.pprint("________________________________________________________________________________")
pp.pprint("После вставки в items")
pp.pprint("------------items-------------")
...
# c.execute('''Update items set i_count = i_count * 2, i_contract = i_contract + 10 
#           where i_id = 2 and i_contract = 100002''')
# c.execute('''Update items set i_count = i_count * 3, i_contract = i_contract - 2 
#           where i_id = 2 and i_contract = 100002''')
c.execute('''Update items set i_count = i_count + 6000 where i_id = 1''')

# После изменения в items
pp.pprint("________________________________________________________________________________")
pp.pprint("После изменения в items")
pp.pprint("------------items-------------")
c.execute('''Select * from items''')
pp.pprint(c.fetchall())
pp.pprint("------------books------------")
...
# Удаление строк заказа
c.execute('''Delete from items where i_id = 2 and i_contract = 100009''')

# После удаления в items
pp.pprint("________________________________________________________________________________")
pp.pprint("После удаления в items")
pp.pprint("------------items-------------")
c.execute('''Select * from items''')
pp.pprint(c.fetchall())
pp.pprint("------------books------------")
...
"""c.execute('''ALTER TABLE posts RENAME TO new_posts''')
c.execute('''select name, sql from sqlite_master''')
pp.pprint(c.fetchall())"""
